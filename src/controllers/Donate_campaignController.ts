...
export class Donate_campaignController {
    async createDonate(req: Request, res: Response) {
        try {
            ...
            await campaignService.updateCampaignDonate(donateData.campaign.toString(), donateData.money);
    ...
    async updateDonate(req: Request, res: Response) {
        try {
            const id = req.body.id;
            const donate = await Donate_campaignService.updateDonateInfo(id, req.body as Partial<IDonateCampaign>);
