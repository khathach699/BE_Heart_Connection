...
export class Member_campaignController {
    async participate(req: Request, res: Response) {
        try {
            ...
            const uncompletedState = await StateSchema.findOne({ name: "Uncompleted" });
            ...
            const result = await Member_CampaignService.saveParticipant(participantData);
            
            await campaignService.updateCampaignParticipant(participantData.campaign.toString());
    ...
    async updateParticipant(req: Request, res: Response) {
        try {
            const id = req.body.id;
            const participant = await Member_CampaignService.updateParticipantInfo(id, req.body as Partial<IMemberCampaign>);
    ...
    async deleteParticipant(req: Request, res: Response) {
        try {
            ...
            const deletedParticipant = await Member_CampaignService.deleteParticipant(id);
            return CreateSuccessResponse(res, 200, {
