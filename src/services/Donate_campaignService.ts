...
export class Donate_campaignService {
  async saveDonatedInfo(donatedInfo: IDonateCampaign): Promise<IDonateCampaign> {
    try {
      ...
      throw new Error(`Error creating donate info: ${(error as Error).message}`);
    }
  }
  async getAllDonateInfo(page: number = 1, limit: number = 10) {
    try {
      ...
      throw new Error(`Error fetching donate info: ${(error as Error).message}`);
    }
  }
  ...
  async updateDonateInfo(
    ...
    try {
      ...
      throw new Error(`Error updating donate info: ${(error as Error).message}`);
    }
  }
