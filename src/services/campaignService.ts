...
export class CampaignService {
    ...
    async updateCampaignDonate(campaignId: string, amount: number): Promise<ICampaignDocument> {
        try {
            const campaign = await Campaign.findById(campaignId);
            if (!campaign) throw new Error("Campaign not found");
            ...
            throw new Error(`Error updating campaign donate: ${(error as Error).message}`);
        }
    }

    async updateCampaignParticipant(campaignId: string): Promise<ICampaignDocument> {
        try {
            const campaign = await Campaign.findById(campaignId);
            if (!campaign) throw new Error("Campaign not found");
            ...
            throw new Error(`Error updating campaign participant: ${(error as Error).message}`);
        }
    }
}
